...
//        'index': the current tree node index
// Effect: helper method returns the node if found; null otherwise
...
// Input: 'bst': a binary search tree
//        'key': the key of the desired node
// Effect: returns the node if found; null otherwise
Data *bstree_search(BStree bst, Key key) {
	return bst_search(bst, bst->root, key);
}

// Input: 'bst': a binary search tree
//        'data': minimum data size that the node needs to hold
//        'index': the current tree node index
// Effect: the address of the key containing the first node whose data is greater than or equal to data.
static Key *bst_data_search(BStree bst, int index, Data data) {
  if (index == 0)
    ...
    else if (bst->tree_nodes[index].data >= data)
...
// Input: 'bst': a binary search tree
//        'data': minimum data size that the node needs to contain
// Effect: calls a helper function to return an address of the key containing the first node whose data is greater than or equal to the desired data.
